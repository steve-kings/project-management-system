...
const CreateProjectDialog = ({ isDialogOpen, setIsDialogOpen }) => {
    ...
    if (!currentWorkspace && isDialogOpen) {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center text-left z-50" onClick={(e) => e.target === e.currentTarget && setIsDialogOpen(false)}>
            <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 w-full max-w-lg text-zinc-900 dark:text-zinc-200 relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        >
                            ...
                            {currentWorkspace?.members
                                ?.filter((member) => !formData.team_members.includes(member.userId._id))
                                .map((member) => (
                                    <option key={member.userId._id} value={member.userId._id}>
                                        {member.userId.name} ({member.userId.email})
                                    </option>
                        ...
                        {formData.team_members.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-2">
                                {formData.team_members.map((userId) => {
                                    const member = currentWorkspace?.members?.find(m => m.userId._id === userId);
                                    return (
                    ...
                    <div className="flex justify-end gap-3 pt-2 text-sm">
                        ...
                        <button type="submit" disabled={isSubmitting || !currentWorkspace} className="px-4 py-2 rounded bg-gradient-to-br from-blue-500 to-blue-600 text-white dark:text-zinc-200 disabled:opacity-50 disabled:cursor-not-allowed" >
