...
const ProjectTasks = ({ tasks }) => {
    ...
    const handleStatusChange = async (taskId, newStatus) => {
        try {
            ...
            const response = await taskAPI.update(task._id || task.id, { status: newStatus });
                            ...
                            <tbody>
                                {filteredTasks.length > 0 ? (
                                    filteredTasks.map((task) => {
                                        ...
                                        return (
                                            <tr key={task.id} onClick={() => navigate(`/taskDetails?projectId=${task.projectId}&taskId=${task.id}`)} className={`border-t border-zinc-300 dark:border-zinc-800 group hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-all cursor-pointer ${task.status === 'DONE' ? 'opacity-60' : ''}`} >
                                                ...
                                                <td className="px-4 pl-0 py-2 flex items-center gap-2">
                                                    <input 
                                                        type="checkbox" 
                    ...
                    <div className="lg:hidden flex flex-col gap-4">
                        {filteredTasks.length > 0 ? (
                            filteredTasks.map((task) => {
                                ...
                                return (
                                    <div key={task.id} className={`dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4 flex flex-col gap-2 ${task.status === 'DONE' ? 'opacity-60' : ''}`}>
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-2 flex-1">
                                                <input 
                                                    ...
                                                    className="size-4 rounded accent-blue-500 cursor-pointer"
                                                />
                                                <h3 className={`text-zinc-900 dark:text-zinc-200 text-sm font-semibold ${task.status === 'DONE' ? 'line-through' : ''}`}>{task.title}</h3>
                                            </div>
